...
export class DatabaseFileHelper {
    ...
    static async createFunctions(params: {
        ...
        for (let t = 0; t < Object.keys(databaseObject.table).length; t++) {
            const tableName = Object.keys(databaseObject.table)[t];
                ...
                for (let i = 0; i < actions.length; i++) {
                    ...
                    if (!databaseObject.table[tableName].tags[`no-${action}`]) {
                        ...
                        for (let j = 0; j < dbParamsFields.length; j++) {
                            ...
                            fileString = fileString.replace(new RegExp(`<${param}>`, 'gi'), dbParams[param]);
                        }
